...
// export default function Library({ onTrackSelect, onAddToPlaylist, currentTrack, isPlaying, playlistTracks }) {
//   return (
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {tracks.map((track) => (
          ...
          >
            ...
            />
            <h3 className="font-semibold text-lg">{track.title}</h3>
            <p className="text-gray-400 dark:text-gray-300">{track.artist}</p>

            <div className="absolute inset-0 flex items-center justify-center bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity">
              <button
                onClick={() => handlePlayClick(track)}
                className="p-3 bg-white dark:bg-gray-700 rounded-full text-black dark:text-white mr-2"
                aria-label={isPlaying && currentTrack?.id === track.id ? "Pause" : "Play"}
              >
                {isPlaying && currentTrack?.id === track.id ? (
                  <PauseIcon className="h-6 w-6" />
              ...
              <button
                onClick={() => handleAddToPlaylist(track)}
                className="p-3 bg-blue-500 dark:bg-blue-600 rounded-full text-white"
                aria-label="Add to playlist"
              >
                {addedTracks[track.id] ? <CheckIcon className="h-6 w-6" /> : <PlusIcon className="h-6 w-6" />}
              </button>
        ...
        {tracks.map((track) => (
          ...
          >
            ...
            <div className="flex items-center">
              <button
                onClick={() => handlePlayClick(track)}
                className="p-2 bg-white dark:bg-gray-700 rounded-full text-black dark:text-white mr-2"
                aria-label={isPlaying && currentTrack?.id === track.id ? "Pause" : "Play"}
              >
                {isPlaying && currentTrack?.id === track.id ? (
                  <PauseIcon className="h-5 w-5" />
              ...
              <button
                onClick={() => handleAddToPlaylist(track)}
                className="p-2 bg-blue-500 dark:bg-blue-600 rounded-full text-white"
                aria-label="Add to playlist"
              >
                {addedTracks[track.id] ? <CheckIcon className="h-5 w-5" /> : <PlusIcon className="h-5 w-5" />}
              </button>
